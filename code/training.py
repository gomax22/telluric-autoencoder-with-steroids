...
class AutoEncoder(nn.Module):
    def __init__(self, num_features, latent_features, order, batch_size, wave):  
    ...
    # --------------------------------------- Decoder ------------------------------------------
        ...
        S   = doppler_shift(self.decoder[0].weight[:,0].unsqueeze(0).repeat(self.batch_size, 1),self.batch_size,self.wave,ref_berv)

        H2O = self.decoder[0].weight[:,1].unsqueeze(0).repeat(self.batch_size, 1)
        
        # If n_dim = 3 include O2
        if self.latent_features==3: 
            O2  = self.decoder[0].weight[:,2].unsqueeze(0).repeat(self.batch_size, 1)
...
if __name__ == '__main__':
    ...
    Path("/projects/data/HARPS/solar/2022/output/").mkdir(parents=True, exist_ok=True)
    
    print('Loading Data...')
    
    # Loading preprocessed data
    # pkl_file = open('../preproc/preproc_wave.pkl', 'rb')
    pkl_file = open('/projects/data/HARPS/solar/2022/preproc/preproc_wave.pkl', 'rb')
    preproc_wave = pickle.load(pkl_file)
    pkl_file.close()

    # pkl_file = open('../preproc/preproc_flux.pkl', 'rb')
    pkl_file = open('/projects/data/HARPS/solar/2022/preproc/preproc_flux.pkl', 'rb')
    preproc_flux = pickle.load(pkl_file)
    pkl_file.close()

    # pkl_file = open('../preproc/preproc_airmass.pkl', 'rb')
    pkl_file = open('/projects/data/HARPS/solar/2022/preproc/preproc_airmass.pkl', 'rb')
    Airmass = pickle.load(pkl_file)
    pkl_file.close()

    # pkl_file = open('../preproc/preproc_berv.pkl', 'rb')
    pkl_file = open('/projects/data/HARPS/solar/2022/preproc/preproc_berv.pkl', 'rb')
    Berv = pickle.load(pkl_file)
    pkl_file.close()

    # Collecting dimension parameters
    n_obs = preproc_flux.shape[0] # Number of observations
    ...
    for order in range(0,preproc_flux.shape[1]):
        ...
        # ------------------------
        learning_rate = learning_rate / (n_obs/batch_size)
        # ------------------------
        ...
        # ============================== Defining layers and model architecture  ======================
         ...
         # Input and output layer dimension is number of bands to training data 
        num_features = len(train[0])
        # Creating network and setting shape of latent space
        net = AutoEncoder(num_features=num_features, latent_features=n_dim, order=order)
        ...
        torch.save({
                ...
                'order': net.order,
                'latent_features': net.latent_features,
                'num_features': net.num_features,
            },  f'/projects/data/HARPS/solar/2022/output/weights_{order}.pt')
    ...
    output = open('/projects/data/HARPS/solar/2022/output/tellurics.pkl', 'wb')
