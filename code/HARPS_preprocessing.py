...
for path, subdirs, files in os.walk("/projects/data/HARPS/solar/2022"):
    for name in files:       
        # Loading Blaze files
...
for k in range(K):
    ...
    # taking the log obv introduces NaNs for negative flux values, so
    # 1. remove spectrum with negative flux values
    # 2. set negative values to epsilon > 0
    # interpol_flux = np.where(interpol_flux < 0, np.finfo(np.float64).eps, interpol_flux) # should not be needed
...
Path("/projects/data/HARPS/solar/2022/preproc/").mkdir(parents=True, exist_ok=True)

output = open('/projects/data/HARPS/solar/2022/preproc/preproc_wave.pkl', 'wb')
pickle.dump(Preproc_wave, output)
output.close()

output = open('/projects/data/HARPS/solar/2022/preproc/preproc_flux.pkl', 'wb')
pickle.dump(Preproc_flux, output)
output.close()

output = open('/projects/data/HARPS/solar/2022/preproc/preproc_airmass.pkl', 'wb')
pickle.dump(Preproc_airmass, output)
output.close()

output = open('/projects/data/HARPS/solar/2022/preproc/preproc_berv.pkl', 'wb')
pickle.dump(Preproc_berv, output)
output.close()

output = open('/projects/data/HARPS/solar/2022/preproc/preproc_const.pkl', 'wb')
pickle.dump(const, output)
output.close()

with open('/projects/data/HARPS/solar/2022/preproc/preproc_fnames.txt', 'w') as f:
    f.write("\n".join(file_name))
